...
void complexquadsolve(double a, double b, double c, Complex& s1){
    double delta = b*b - 4 * a * c;
    s1.re = (-b) / (2 * a); 
    s1.im = (std::sqrt(abs(delta))) / (2*a); 
}

std::string complex_to_string(Complex s){
    return std::to_string(s.re) + " +- i" + std::to_string(s.im); 
}
...
int main(){
    ...
    if(quadsolve(a, b, c, s1, s2)){
        ...
        Complex z; 
        std::string cmpans; 
        complexquadsolve(a, b, c, z); 
        cmpans = complex_to_string(z); 
        std::cout << "there are no real , the complex solutions are:" << std::endl;
        std::cout << cmpans << std::endl; 
    }
