...
int main(){
    ...
    std::cout << "adding another node to the list we get:" << std::endl; 
    ...
    lit = l; 
    //we have to write lit = l here instead of before as if we wrote it before, it wouldn't take into account the new node added as it is still focusing on the l before the change
    while(lit != NULL){
        std::cout << (*lit).val << std::endl;
        lit = (*lit).next;
    }
